...
def move_table(catalog,f,t):
    sys_cols=['RID','RCT','RCB','RMB','RMT']
    from_sname,from_tname = f
    to_sname,to_tname = t

    from_cols = [i.name  for i in catalog.getCatalogModel().schemas[from_sname].tables[from_tname].column_definitions if not i.name in sys_cols]
